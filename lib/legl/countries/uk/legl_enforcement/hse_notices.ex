defmodule Legl.Countries.Uk.LeglEnforcement.HseNotices do
  ...
  """
  require Logger
  alias Legl.Services.Hse.Client
  ...
  defp get_hse_notices(page, opts) when is_binary(page) do
    ...
    notices = Client.get_hse_notices(page_number: page, country: opts.country)
      ...
      Enum.map(
        ...
        &Map.merge(
          %HSENotice{
            ...
            offender_business_type: offender_business_type(&1.offender_name),
            offender_index: offender_index(&1.offender_name)
          },
          &1
        )
      )
    ...
    notices =
      Enum.reduce(notices, [], fn
        %HSENotice{regulator_id: regulator_id} = notice, acc when regulator_id != "" ->
          ...
          |> Map.merge(Client.get_notice_details(regulator_id))
          |> Map.merge(Client.get_notice_breaches(regulator_id))
          |> (&[&1 | acc]).()
  ...
  defp offender_business_type(offender_name) do
    cond do
  ...
  defp offender_index(offender_name) do
  ...
  def get_linked_airtable_record_id(%{title: title} = breach) do
